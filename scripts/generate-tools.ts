...
function buildToolDef(
  ...
  for (const param of resolvedParams) {
    ...
    let zodType = jsonSchemaToZodString(
      paramSchema as Parameters<typeof jsonSchemaToZodString>[0],
      {
        rootSpec: spec,
      },
    );
  ...
  if (bodySchema) {
    ...
    if (resolved.allOf) {
      ...
      for (const sub of resolved.allOf as Record<string, unknown>[]) {
        ...
        if (subResolved.required) {
          mergedRequired.push(...(subResolved.required as string[]));
        }
      }
      effectiveSchema = {
        type: "object",
    ...
    if (effectiveSchema.type === "object" && effectiveSchema.properties) {
      ...
      for (const [key, propSchema] of Object.entries(props)) {
        if (pathParams.includes(key) || queryParams.includes(key)) {
          continue;
        }

        let zodType = jsonSchemaToZodString(
          propSchema as Parameters<typeof jsonSchemaToZodString>[0],
          {
            rootSpec: spec,
          },
        );
...
function main() {
  ...
  const spec = JSON.parse(readFileSync(SPEC_PATH, "utf-8")) as Record<
    string,
    unknown
  >;
  ...
  for (const tool of tools) {
    ...
    const desc =
      tool.summary + (tool.description ? ` — ${tool.description}` : "");
